...
int do_lseek(int fd, int offset, int whence){
    ...
    switch(whence){
       case SEEK_SET: cur_off = 0;
          break;
       case SEEK_CUR: cur_off = *pos;
          break;
       case SEEK_END: cur_off = cur_process->file[fd]->inode->sn_size;
          break;
    }
    
    //error
    if(cur_off + offset > cur_process->file[fd]->inode->sn_size 
          && cur_off + offset < 0) return -1;
    *pos =cur_off + offset;
    return *pos;
}
